...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
    ...
    int n = in_list->len;
    for (int i = 0; i < n; i++) {
        element_t* out = malloc(sizeof(element_t)); // Allocate memory for the output element
        if (out == NULL) {
            // Handle memory allocation failure here
            // For simplicity, let's just exit the loop and return
            return;
        }
        ...
        if (*out == NULL) {
            *out = malloc(sizeof(**out));
            if (*out == NULL) {
                ...
                // For simplicity, let's return without freeing memory
                return;
            }
        }
