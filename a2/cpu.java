    ...
    protected void execute() throws InvalidInstructionException, MachineHaltException, RegisterSet.InvalidRegisterNumberException, MainMemory.InvalidAddressException {
        switch (insOpCode.get()) {
            ...
            case 0x1: // ld o(rs), rd .......... 1psd  (p = o / 4)
                int address = reg.get(insOp1.get()) + (insOp0.get() << 2);
                reg.set(insOp2.get(), mem.readInteger(address));
                break;
            case 0x2: // ld (rs, ri, 4), rd .... 2sid
                int addressIndexed = reg.get(insOp0.get()) + (reg.get(insOp1.get()) << 2);
                reg.set(insOp2.get(), mem.readInteger(addressIndexed));
                break;
            case 0x3: // st rs, o(rd) .......... 3spd  (p = o / 4)
                int addressStore = reg.get(insOp2.get()) + (insOp0.get() << 2);
                mem.writeInteger(addressStore, reg.get(insOp1.get()));
                break;
            case 0x4: // st rs, (rd, ri, 4) .... 4sdi
                int addressStoreIndexed = reg.get(insOp1.get()) + (reg.get(insOp2.get()) << 2);
                mem.writeInteger(addressStoreIndexed, reg.get(insOp0.get()));
                break;
            case 0x6: // ALU ................... 6-sd
                switch (insOp0.get()) {
            ...
            case 0x7: // sh? $i,rd ............. 7dii
                int shiftValue = insOp1.get();
                if (shiftValue >= 0 && shiftValue < 8) {
                    // Shift right logical
                    reg.set(insOp2.get(), reg.get(insOp2.get()) >>> shiftValue);
                } else if (shiftValue >= 8 && shiftValue < 16) {
                    // Shift left logical
                    reg.set(insOp2.get(), reg.get(insOp2.get()) << (shiftValue - 8));
                } else {
                    throw new InvalidInstructionException();
                }
                break;
