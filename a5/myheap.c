...
static void *coalesce(struct myheap *h, void *first_block_start) {
  ...
  if (!is_within_heap_range(h, next_block_start)) {
    return NULL;  // Cannot coalesce, next block is outside the heap
  }
  ...
  if (!block_is_in_use(first_block_start) && !block_is_in_use(next_block_start)) {
    ...
    int new_block_size = get_block_size(first_block_start) + get_block_size(next_block_start);

    // Update the header and trailer of the first block
    set_block_header(first_block_start, new_block_size, 0);

    // If the next block is not the last block, update its trailer as well
    if (!is_last_block(h, first_block_start)) {
      void *next_block_trailer = next_block_start + get_block_size(next_block_start) - HEADER_SIZE;
      *((long*)next_block_trailer) = new_block_size;  // Cast to the appropriate type
    }

    return first_block_start;
  }

  return NULL;  // Coalesce not possible
...
static int get_size_to_allocate(int user_size) {
  ...
  /* TO BE COMPLETED BY THE STUDENT. */
   int total_size = user_size + HEADER_SIZE * 2;
...
static void *split_and_mark_used(struct myheap *h, void *block_start, int needed_size) {
  ...
  if (block_size - needed_size >= 3 * HEADER_SIZE) {
    ...
    int new_block_size = needed_size;
    int remaining_size = block_size - new_block_size;

    // Mark the first block as in use
    set_block_header(block_start, new_block_size, 1);

    // Mark the second block as free
    void *next_block_start = block_start + new_block_size;
    set_block_header(next_block_start, remaining_size, 0);

    return get_payload(block_start);
  } else {
    // Mark the entire block as in use
...
void myheap_free(struct myheap *h, void *payload) {
  ...
  if (!is_first_block(h, block_start)) {
    ...
    if (!block_is_in_use(prev_block_start)) {
      block_start = coalesce(h, prev_block_start);
    }
  }
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  ...
  /* TO BE COMPLETED BY THE STUDENT. */
  int size_to_allocate = get_size_to_allocate(user_size);

  // Start from the beginning of the heap
  void *current_block = h->start;

  while (is_within_heap_range(h, current_block)) {
    if (!block_is_in_use(current_block) && get_block_size(current_block) >= size_to_allocate) {
      // Found a free block large enough
      return split_and_mark_used(h, current_block, size_to_allocate);
    }
